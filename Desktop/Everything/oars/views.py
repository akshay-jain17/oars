...
def index(request):
	user = request.user
	done_courses_list = DoneCourses.objects.filter(user = user)
...
def add_course(request):
	...
	if request.method == 'POST':
		add_course_form = CourseOfferForm(request.POST)
		if add_course_form.is_valid():
			clean_form_data = add_course_form.cleaned_data['course_no']
			CourseOffer.add(
				unicode(os.urandrom(32)),
				clean_form_data.course_no,
				clean_form_data.course_instructor,
				clean_form_data.course_note,
				clean_form_data.course_time
			)
			return HttpResponseRedirect('/user_page.html')
		else:
			add_course_form = CourseOfferForm()
		variables = RequestContext(request, {
		'add_course_form': add_course_form
		})
		return render_to_response('add_course_page.html', variables)
	else:
		add_course_form = CourseOfferForm()
	variables = RequestContext(request, {
	'add_course_form': add_course_form
